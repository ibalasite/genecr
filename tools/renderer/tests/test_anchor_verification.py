...
def _load_baseline():
    ...
    sa = json.loads((BASELINE / "spec-advanced.input.json").read_text(encoding="utf-8"))
    return sb, sa


def test_baseline_server_eq_10():
    sb, sa = _load_baseline()
    f = _per_role_points_anchor(sb, sa)
...
def test_anchor_scale_example_red_envelope_server():
    ...
    sb = {"resource_counts": {"api_endpoints": 10}}
    sa = {"data_models": [{"kind": "mysql"}] * 5 + [{"kind": "redis"}] * 6}
    f = _per_role_points_anchor(sb, sa)
    # 10 × 5/8 + 5 × 3/4 + 6 × 2/4 = 6.25 + 3.75 + 3.0 = 13.0
    assert abs(f["server_engineer"] - 13.0) < 0.01


def test_spec_basic_step_without_sa_treats_mysql_redis_as_zero():
    """spec-basic 步驟跑公式時 sa 還沒生 → mysql/redis 計 0，server 只算 API"""
    sb = {"resource_counts": {"api_endpoints": 8}}
    f = _per_role_points_anchor(sb, None)
    # 只算 API: 8 × 5/8 = 5.0
