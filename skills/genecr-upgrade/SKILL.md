...
- 路徑含 `.gemini` → host = **gemini** → `GENECR_DIR=$HOME/.gemini/skills/genecr`
- 路徑含 `.copilot` → host = **copilot** → `GENECR_DIR=$HOME/.copilot/skills/genecr`
...
GENECR_DIR="$HOME/.claude/skills/genecr"   # ← 若從 .codex / .gemini / .copilot，改成對應 host 的 skills/genecr
...
> 同時升所有 host（claude + codex + gemini + copilot）：`bash "$GENECR_DIR/setup" upgrade all`
