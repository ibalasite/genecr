...
## ⚠️ SPECIAL RULE — `output_too_small`

If the issues list contains an issue with `"category": "output_too_small"`:
- The ORIGINAL INPUT below is **invalid** — the generator completely failed
  (output was too small relative to the prompt, e.g. "請授權寫入檔案")
- **DO NOT** use the original input as a base; ignore it entirely
- **REGENERATE** the complete document from scratch using the brief (in
  upstream), the schema shape, and any upstream outputs
- Treat this exactly like a fresh generator call, not a patch
