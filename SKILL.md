...
**結構必須對齊既有企畫範本（以下為章節骨架，**直接照抄**，不要自創或省略）**：

```
1. 修改紀錄（日期 / 修改人 / 內容 表格，初版填 YYYY-MM-DD）
2. 相關文件（檔案 / 說明 / 狀態 表格，含 figma 連結佔位）
3. 概要（一段話：功能是什麼、為什麼做、目標玩家族群，≤ 3 行）
4. 競業分析摘要（各市場代表案例 + 來源 URL；功能對比表；值得借鑒亮點）
5. 分類（多軸切割）
   5.1 軸 1：類別（窮舉選項，例：REELS / FISH / OTHERS / JP）
   5.2 軸 2：數據指標（窮舉選項，例：單局贏分 / 單局倍率）
   5.3 軸 3：時間區段（窮舉選項，例：7日 / 30日 / 歷史）
6. 功能對照表（軸交叉的 cross-product 表格，標 ✓ 或 -- 不適用）
7. 顯示欄位定義（每欄一行：名稱 + 格式範例如 8,888,888 千分位 + 條件顯示規則）
8. 上榜規則 / 業務規則（觸發條件、重複處理、上限筆數、排序 tiebreaker、防作弊）
9. 用戶旅程（Step-by-step，每步註記情緒/動機/關鍵決策點）
10. 介面說明（編號條列）
    10.1 第一層切換（位置 / 觸發方式 / 選項）
    10.2 第二層切換（…）
    10.3 時間切換（下拉 / 標籤 / 預設值）
    10.4 資料區（捲動方式 / 顯示筆數上限 / 空狀態文案）
    10.5 我的 XX（底部固定 / 永久可見 / 未達門檻文案）
    10.6 刷新機制（固定週期？事件觸發？延遲容忍？）
    10.7 擴充性註記（預留哪些欄位 / 空間）
    10.8 錯誤與邊界狀態（斷網、無資料、權限不足）
11. 文案對照表（至少繁中 / 英 / 西 三語，東南亞市場視需要加越/泰/印尼，逐個 UI 詞條一列）
12. 說明頁（給玩家看的圖文版：1 句 hook、3–5 個 emoji bullet、操作步驟，可貼遊戲內 Help/FAQ）
13. 建議上線時程（Phase 1/2/3 表格，標註「⚠️ 以下為建議，請企畫依實際團隊資源調整」）
14. 資源需求概覽（X 圖 / Y 動畫 / Z 音效 / 工程複雜度 低/中/高 + 原因）
```

**章節 11（文案對照表）參考骨架**（沿用排行榜範本的詞條風格，按功能替換）：
...
| 老虎機 | REELS | Tragamonedas |
| 捕魚機 | FISH | Pesca |
| 大廳彩金 | JP | JP |
| 單局贏分榜 | Total Win | Mayor Ganancia |
| 單局倍率榜 | Highest Multiplier | Mayor Multiplicador |
| 七日排行 | 7 Days | 7 Días |
| 歷史排行 | All-Time | Histórico |
| 排名 | Rank | Rango |
| 玩家 ID | Player ID | ID de Jugador |
| 遊戲 | Game | Juego |
| 贏分 | Win | Ganancia |
| 倍率 | Multiplier | Multiplicador |

> 翻譯原則：iGaming 玩家慣用詞優先（不照字面翻），保留品牌語氣。每個 UI 詞條都要在這張表，不要漏。
